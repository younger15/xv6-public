...
int main(int argc, char *argv[])
{
  17:	81 ec 10 04 00 00    	sub    $0x410,%esp


	int fd = open("UserList", O_RDONLY);
  1d:	6a 00                	push   $0x0
  1f:	68 80 07 00 00       	push   $0x780
  24:	e8 09 03 00 00       	call   332 <open>
	//char *targetUser = argv[1];
	//printf(1,"user: %s\n", targetUser);
	//char nameList[128];
	//char *names = nameList;
...
int main(int argc, char *argv[])
{
  ...
  2c:	89 c7                	mov    %eax,%edi
	...
	char buf[1024];
	int checkReadFile = read(fd, buf,sizeof buf);
  2e:	68 00 04 00 00       	push   $0x400
  33:	56                   	push   %esi
  34:	50                   	push   %eax
  35:	e8 d0 02 00 00       	call   30a <read>
	if(checkReadFile < 0)
  3a:	83 c4 10             	add    $0x10,%esp
  3d:	85 c0                	test   %eax,%eax
  3f:	79 24                	jns    65 <main+0x65>
  41:	eb 3e                	jmp    81 <main+0x81>
  43:	90                   	nop
  ...
  62:	83 c6 01             	add    $0x1,%esi
		...
		{
			c = buf[i];
  65:	0f b6 1e             	movzbl (%esi),%ebx
			i++;
			if (c==':')
  68:	80 fb 3a             	cmp    $0x3a,%bl
  6b:	75 db                	jne    48 <main+0x48>
				printf(1,"\n");
  6d:	83 ec 08             	sub    $0x8,%esp
  70:	68 9d 07 00 00       	push   $0x79d
  75:	6a 01                	push   $0x1
  77:	e8 e4 03 00 00       	call   460 <printf>
  7c:	83 c4 10             	add    $0x10,%esp
  7f:	eb e1                	jmp    62 <main+0x62>
	//char *names = nameList;
	char buf[1024];
	int checkReadFile = read(fd, buf,sizeof buf);
	if(checkReadFile < 0)
  ...
  81:	50                   	push   %eax
  82:	50                   	push   %eax
  83:	68 89 07 00 00       	push   $0x789
  88:	6a 01                	push   $0x1
  8a:	e8 d1 03 00 00       	call   460 <printf>
...
close(fd);
  ...
  9b:	57                   	push   %edi
  9c:	e8 79 02 00 00       	call   31a <close>
exit();
...
printint(int fd, int xx, int base, int sgn)
{
 ...
 3ff:	f7 f6                	div    %esi
 401:	0f b6 92 ac 07 00 00 	movzbl 0x7ac(%edx),%edx
  }while((x /= base) != 0);
...
putc(int fd, char c)
{
 ...
 578:	89 45 d0             	mov    %eax,-0x30(%ebp)
        if(s == 0)
          s = "(null)";
 57b:	b8 a2 07 00 00       	mov    $0x7a2,%eax
